...
def plot_tensor_slices(T, T_hat, cmap="viridis"):
    """
    Display all 2-D slices of a high-order tensor side-by-side.

    For a tensor with shape (n1, n2, …, n_{k-2}, n_{k-1}, n_k) the function
    flattens the first k-2 modes, giving  n1·n2·…·n_{k-2}   slices,
    each of size  (n_{k-1} x n_k).

    Parameters
    ----------
    T, T_hat : np.ndarray
        Original tensor and its reconstruction (same shape).
    cmap     : str, optional
        Matplotlib colour-map for imshow.
    """
    if T.shape != T_hat.shape:
        raise ValueError("T and T_hat must have identical shapes")

    if T.ndim < 2:
        raise ValueError("Tensor must have at least two modes")

    # reshape to (num_slices, n_{k-1}, n_k)
    n_slices = int(np.prod(T.shape[:-2]))
    h, w     = T.shape[-2:]

    A  = T.reshape(n_slices,  h, w)
    A_ = T_hat.reshape(n_slices, h, w)
    ...
    for idx in range(n_slices):
        ...
        plt.subplot(1,2,1)
        plt.imshow(A[idx], aspect='auto')
        plt.title(f"Original slice {idx}")
        plt.colorbar()

        plt.subplot(1,2,2)
        plt.imshow(A_[idx], aspect='auto')
